...
class FavouriteController extends Controller
{
    ...
    public function store(Request $request)
    {
        if($request->has('estate_id')){
            ...
            return auth()->user()->favourites()->whereOfficeId($request->office_id)->exists() ? auth()->user()->favourites()->whereOfficeId($request->office_id)->delete() : auth()->user()->favourites()->create($request->all());
        }else{
